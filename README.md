...
## Reproduce
    ...
    python -m math_statistics.run_all       # screening + corrected tables + all 19 figures
...
### Figures
`python -m math_statistics.run_all` regenerates all 19 figures from `data/features/master_dataset_old.csv` into `math_statistics/output/plots/`. They are **byte-identical across independent runs** — verified by SHA-256 over two full runs (all 19 PNG *and* all 19 TIFF matched). Determinism follows from fixed seeds (`numpy.random.default_rng(42)` for the bootstrap-CI panel, `random_state=42` for the VIF subsample) and otherwise data-only plotting. The committed 300-dpi PNGs are shown in [Figures](#figures) below; the 600-dpi LZW-TIFF variants regenerate identically but are git-ignored (~54 MB). One panel (derived-soil top correlations, slot 19) is skipped when its input table is empty, so 19 of the 20 defined panels are produced. The standalone `python -m math_statistics.plots` entry point now reads the same `FEATURES_CSV` as the rest of the pipeline (it previously referenced a private `full_dataset.csv` that is not shipped in this repository).
...
**Fig. 13 — Top variance-inflation factors (multicollinearity screen; VIF > 10 flagged).**

![Fig. 13](math_statistics/output/plots/13_vif_multicollinearity.png)
